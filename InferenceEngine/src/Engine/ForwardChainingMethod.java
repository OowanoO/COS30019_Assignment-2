...
public class ForwardChainingMethod extends LogicAlgorithms {
    public ForwardChainingMethod(KnowledgeBase kb, String ask) {
        super(kb, ask);
        setShortName("FC");
    ...
    public String testInput() 
    {
        ...
        // Create an inferred list to keep track of facts that have already been inferred
        ArrayList<String> inferred = new ArrayList<String>();

        // Create a counted array to keep track of which Horn clauses have already been counted
        ...
        while (!agenda.isEmpty()) 
        {
            ...
            if (!inferred.contains(p)) 
            {
                ...
                for (int i = 0; i < getKnowledgeBase().getHornClause().size(); i++) 
                {
                    ...
                    // If the Horn clause has not already been counted and its literals contain the fact
                    if (!counted[i] && hClause.getLiterals().contains(p)) 
                    {
                        ...
                        if (hClause.countLiterals() == 0) {
                            ...
                            if (hClause.getInferred().equals(getInput())) {
                                ...
                                // Build and return a string containing "YES" and a comma-separated list of all literals in the inferred list
    ...
    // Method that checks if the question being tested is already a known fact in the knowledge base
    @Override
